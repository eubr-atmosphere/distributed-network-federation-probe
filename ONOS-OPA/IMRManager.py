...
class IMRManager(object):
    ...
    def easy_reroute(self, tm, topoManager):
        reroute_msg = {'routingList': []}
        topo = topoManager.G.copy()

        for link in topo.edges():
            #print("For link %s, insert latency in ms:", link)
            topo[link[0]][link[1]]["latency"] = 1

        try:
            if self.slow_critical:
                a = topo.__getitem__(topoManager.hosts[0]).keys()[0]
                b = topo.__getitem__(topoManager.hosts[1]).keys()[0]
                topo[a][b]["latency"] = 100
                print("Slowing direct path -> expect rerouting! ")
                self.slow_critical = False
            else:
                print("Fast direct path -> expect rerouting! ")
                self.slow_critical = True
        except:
            pass
        ...
        for flow_id, amount in tm.items():
            ...
            try:
                path = nx.shortest_path(topo, in_elem, out_elem, 'latency')
